...
export function buildOrchestratorSystemPrompt(state: AgentStateType): string {
  return [
    ...
    "- search_video_transcript: PRIMARY tool for content questions — finds the most relevant chunks by semantic similarity.",
    "  If results are empty or similarity scores are low (< 0.5), retry with a rephrased query or lower the threshold (e.g. 0.3).",
    "  After finding relevant chunks, use read_video_transcript with startTime/endTime to read surrounding context if needed.",
    "- read_video_transcript: use for time-range reads or context expansion around a known timestamp.",
